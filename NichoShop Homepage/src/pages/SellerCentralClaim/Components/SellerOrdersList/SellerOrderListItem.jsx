...
const SellerOrderListItem = (props) => {
  const { items } = props;
    ...
    <div className="list-items">
      {currentItems.map((item, index) => (
        <div className="list-item" key={index}>
          <h2
            className={
              titleType[index] == "" ? "title-date border-0" : "title-date"
            }
          >
            <span className={item.active ? "red" : ""}>{titleType[index]}</span>
          </h2>
          <div>
            <div className="d-xl-flex px-3">
              <div xl={2} className="list-left">
                <h3>Status:</h3>
              ...
              <div xl={10} className="w-100">
                <div className="list-right">
                  <div className="d-xl-flex justify-content-between">
                    <div xl={9} className="list-reason">
                      <h3>Reason:</h3>
                      <p className="not">Not received item</p>

                      {item.items.map((item, index) => (
                        <div className="list-detail d-flex" key={index}>
                          <div className="image">
                            <img src={item.items} />
                          </div>
                          <div className="item-detail">
                            <p className="item-title">{item.title}</p>
                            <p className="item-desc">{item.descr}</p>
                          </div>
                        </div>
                      ))}
                    </div>
