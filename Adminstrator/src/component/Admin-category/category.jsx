...
const Category = () => {
    ...
    useEffect(() => {
        Axios.get('https://nichoshop.com/api/v1//admin/category/all')
            .then(res => {
    ...
    const AddSubCategory = (id) => {
        Axios.post('https://nichoshop.com/api/v1/admin/category/parent/' + id, { name: SubCategoryValue })
            .then(res => {
    ...
    // }
    const AddCategory = () => {
        Axios.post('https://nichoshop.com/api/v1/admin/category/parent/0', { name: CategoryValue })
            .then(res => {
                if (res.status == 200) {
                    <Alert_Success />
                }
    ...
    const SetReName = (id, name) => {
        Axios.put('https://nichoshop.com/api/v1/admin/category/rename/' + id + '/' + name)
            .then(res => {
                if (res.status == 200 && res.data == 'OK') {
                    <Alert_Success />
                }
        ...
        <React.Fragment>
                                ...
                                <div key={index} className="accordion-item my-3 ">
                                    <h2 className="accordion-header" id={index} >
                                        <div className="dropdown pull-right">
                                            <div>
                                                <img src='assets/img/plus-circle (1).svg' className='category-dropdown' onClick={() => rename(category.category.name)} data-bs-toggle="dropdown" />
                                        ...
                                        </div>
                                        {
                                            toggleAccordionButton !== index ?
                                                ...
                                                <button className="accordion-button collapsed text-white p--20" onClick={() => toggle(index)} type="button" data-bs-toggle="collapse" data-bs-target={'#' + category.category.name} aria-expanded="false" aria-controls={category.category.name}>
                                                    <img src='assets/img/plus-circle.png' />&nbsp;<b style={{ 'color': 'black' }}>{category.category.name}</b>
                                                </button> :
                                                <button className="accordion-button collapsed text-white p--20" onClick={() => toggle(null)} type="button" data-bs-toggle="collapse" data-bs-target={'#' + category.category.name} aria-expanded="false" aria-controls={category.category.name}>
                                                    <img src='assets/img/minus.png' style={{ 'borderRadius': '50%' }} />&nbsp;<b style={{ 'color': 'black' }}>{category.category.name}</b>
                                                </button>
                                        }
                                    </h2>
                                    <div id={category.category.name} className="accordion-collapse collapse" aria-labelledby={index} data-bs-parent="#accordionExample">
                                                            ...
                                                            const result = (
                                                                <div className='row' key={index} >
                                                                    ...
                                                                    <div className='col-10'>
                                                                        <span>&nbsp;<b>{category.category.name}</b></span>
                                                                    </div>
                                                                    <div className='col-2'>
                                                                        <div>
                                                                            <i className='fa fa-bars' onClick={() => rename(category.category.name)} data-bs-toggle="dropdown" ></i>
                                                                            <ul className="dropdown-menu">
                                                                                <li data-bs-toggle="modal" data-bs-target="#myModal">
                                                                                    <a className="dropdown-item" href="#" >
                                                                                        <img src='assets/img/edit-2.svg' />
                                                ...
                                                <div className='row mt-3'>
                                                    <div className='btn-group'>
                                                        <div className='col-lg-8 col-sm-8 col-sm-8'>
                                                            <input type='text' name='addSubCategory' placeholder='Add a sub-category' onChange={addCategoryValue} className='form-control' />
                                                        </div>
                                                        <div className='col-4 mx-1'>
                                                            <button className='btn btn-outline-success' onClick={() => AddSubCategory(category.category.id)}> <i className='fa fa-plus'></i>&nbsp;Add</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <div className="modal-dialog mt-5">
                    <div className="modal-content p-3">
                        ...
                        <div className="modal-body">
                            ...
                            <input type="text" className='form-control' name='setrename' onChange={addCategoryValue} />
                        </div>
                        <div className="modal-footer">
                            <button type="button" className="btn btn-outline-dark" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" className="btn btn-primary" onClick={() => SetReName(category.category.id, categoryrename)} data-bs-dismiss="modal">Confirm</button>
                        </div>
            ...
            <div className="modal mt-5" id="myModal1">
                <div className="modal-dialog mt-5">
                    <div className="modal-content p-3">
                        <div className="modal-header">
                            <h5 className="modal-title">Move iPod and MP3 Players Accessories to:</h5>
                        </div>
                        <div className="modal-body">
                            {category.map((category, index) => {
                                const list = (
                                    <p key='index'>
                                        {
                                            category.children && category.children ? <span className='fa fa-plus' >&nbsp;{category.category.name}</span> : ''
                                        ...
                                        }
                                        {
                                            category.children.map((category, index) => {
                                                const data = (
                                                    <p className='mx-3'>
                                                        <i className='fa fa-mius'></i> <b>{category.category.name}</b>
                                                    </p>
                                                )
                                                return data;
                                            })
                        ...
                        <div className="modal-body">
                            <span className="modal-title mb-1">Rename to:</span>
                            <input type="text" className='form-control' placeholder='Move to' />
                        </div>
                        <div className="modal-footer">
                            <button type="button" className="btn btn-outline-dark" data-bs-dismiss="modal">Cancel</button>
